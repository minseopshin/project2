...
<div class="topmenu" id="top${top.id}">
 <a href="plist2?category1=${top.category1}&category2=${top.category2}&fb=0" onmouseover="topmenu_change1(${top.id})" onmouseout="topmenu_change2(${top.id})" id="topmenu${top.id}" style="color:black;text-decoration:none" name="category2">
...
<div id="right">
<div class="pdt">
<c:forEach items="${plist3}" var="bdto">
<div class="pdts">
 <a class="box" href="readnum?id=${bdto.id}&chk=1&fb=0" style="color:black;text-decoration:none">
	<img src="${bdto.prd_img}" width="270px" height="270px"> <p>
	<span id="name"> ${bdto.prd_name} </span> <p>
...
<div id="pager" style="display:inline-block;width:1500px;">
	  <!-- 이전 페이지 이동 -->
         <!-- 10페이지 이전 이동 시작 -->
          <c:if test="${pstart != 1}">
           <a href="plist3?category1=${category1}&page=${pstart-1}" style="color:gray;text-decoration:none"> <span style="color:gray;text-decoration:none"> <<&nbsp; </span>  </a>
          </c:if>
          <c:if test="${pstart == 1}">
            << &nbsp;
          </c:if>
         ...
         <c:if test="${page > 1}">
          <a href="plist3?category1=${category1}&page=${page-1}" style="color:gray;text-decoration:none"> < &nbsp; </a>  
         </c:if>
         <c:if test="${page == 1}"> 
          < &nbsp;      
         </c:if>
       ...
       <c:forEach begin="${pstart}" end="${pend}" var="i">
        <c:if test="${page == i}">
         <a href="plist3?category1=${category1}&page=${i}" style="color:#FF9436;text-decoration:none"> ${i}&nbsp; </a>
        </c:if>
        <c:if test="${page != i}">
         <a href="plist3?category1=${category1}&page=${i}" style="color:gray;text-decoration:none"> ${i}&nbsp; </a>
        </c:if>
       </c:forEach>
       <!-- 다음 페이지 이동 -->
        <!-- 다음 1페이지 이동 -->
        <c:if test="${page != pagecnt }">
         <a href="plist3?category1=${category1}&page=${page+1}" style="color:gray;text-decoration:none"> <span style="color:gray;text-decoration:none"> >&nbsp; </span> </a>
        </c:if>
        <c:if test="${page == pagecnt}"> 
          >&nbsp;
        </c:if>
        ...
        <c:if test="${pend != pagecnt }">
         <a href="plist3?category1=${category1}&page=${pend+1}" style="color:gray;text-decoration:none"> <span style="color:gray;text-decoration:none"> >> </span> </a>
        </c:if>
        <c:if test="${pend == pagecnt}">
         >> 
        </c:if>
