...
<hr>
			...
			<ul class="tab1_1" style="list-style-type:none;">
				...
				<li><a href="listanswer" class="active">1:1문의</a></li>
...
<div class="my_reserve_top" >

<img src="resources/img/qnaimg.jpg" border=0  style="width:1000px;height:300px;">
</div>

<div class="my_reserve_left" style="display:inline-block;margin-top:80px;">

<img src="resources/img/listinfo.png" border=0  >
</div>
<div class="my_reserve_wrap"  style="display:inline-block;">
  <table class="my_reserve" align="center">
     ...
     <tr> <!-- 검색행 -->
      <td colspan="4">
        <form method="post" action="listanswer">
                        선택 <select name="cla">
   ...
   <c:forEach items="${listanswer}" var="bdto2">  
    <tr>
    <td> ${bdto2.name} </td>
     <td class="row_question"> 

       <a href="readnumanswer?id=${bdto2.id}" > ${bdto2.title}   </a>

     </td>

     <td> ${bdto2.readnum} </td>
     <td> ${bdto2.writeday} </td>
    </tr>
   </c:forEach> 
<tr>
      <td colspan="4" align="center">
       <!-- 이전 페이지 이동 -->
         <!-- 10페이지 이전 이동 시작 -->
          <c:if test="${pstart != 1}">
           <a href="listanswer?page=${pstart-1}"> << </a>
          </c:if>
          <c:if test="${pstart == 1}">
           <<
          </c:if>
         ...
         <c:if test="${page > 1}">
          <a href="listanswer?page=${page-1}"> < </a>  
         </c:if>
         <c:if test="${page == 1}"> 
          <      
         </c:if>
       ...
       <c:forEach begin="${pstart}" end="${pend}" var="i">
        <c:if test="${page == i}">
         <a href="listanswer?page=${i}" style="color:red;"> ${i} </a>
        </c:if>
        <c:if test="${page != i}">
         <a href="listanswer?page=${i}"> ${i} </a>
        </c:if>
       </c:forEach>
       ...
       <!-- 다음 페이지 이동 -->
        ...
        <c:if test="${pend != pagecnt }">
         <a href="listanswer?page=${pend+1}"> >> </a>
        </c:if>
        <c:if test="${pend == pagecnt}">
          >>
        </c:if>
